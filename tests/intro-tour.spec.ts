...
test.describe("intro tour", () => {
  test.beforeEach(async ({ context, page }) => {
    ...
    await mockClientEndpoints(page);
  });
  ...
  test("auto-runs on first visit and marks all steps seen on completion", async ({ page }) => {
    ...
    await expect(page.locator(".driver-popover.app-tour-popover")).toHaveCount(0);
  ...
  test("clicking the backdrop does not close the tour", async ({ page }) => {
    ...
    await expect(page.locator(".driver-popover.app-tour-popover")).toBeVisible();
  });
  ...
  test("migrates the pre-rename v2:tours:seen map without resetting the user", async ({
    context,
    page,
  }) => {
    await seedLegacySeenTours(context, { intro: INTRO_STEPS });
    await page.goto("/");
    await page.waitForTimeout(900);

    // Every intro step was already seen under the old key, so nothing auto-runs.
    await expect(page.locator(".driver-popover.app-tour-popover")).toHaveCount(0);

    const seen = await readSeen(page);
    expect(seen.intro).toEqual(INTRO_STEPS);
    const legacy = await page.evaluate(() => window.localStorage.getItem("v2:tours:seen"));
    expect(legacy).toBeNull();
  });
