...
/**
 * Seed the `location` cookie so the app renders without hitting the IP lookup.
...
/**
 * Write `tours:seen` localStorage before the first navigation. Playwright
...
export async function seedSeenTours(context: BrowserContext, seen: SeenMap): Promise<void> {
  await context.addInitScript((payload) => {
    window.localStorage.setItem("tours:seen", JSON.stringify(payload));
  }, seen);
}
...
/**
 * Mock the client-side API endpoints via route handlers. Call before the
 * first navigation.
 */
export async function mockClientEndpoints(page: Page): Promise<void> {
...
export async function readSeen(page: Page): Promise<SeenMap> {
  return page.evaluate(() => {
    const raw = window.localStorage.getItem("tours:seen");
...
export async function waitForTourPopover(page: Page): Promise<void> {
  await page.locator(".driver-popover.app-tour-popover").waitFor({ state: "visible" });
}
