...
/**
 * Clear translation cache when source (en.json) hash changed (cookie !== current).
 * Entries under the pre-rename prefix are dropped unconditionally so they do not
 * linger in localStorage forever; the next fetch repopulates the current prefix.
 */
function clearTranslationCacheIfStale(
  ...
  if (typeof window === "undefined") return;
  removeKeysWithPrefix(LEGACY_I18N_CACHE_PREFIX);
  if (clientI18nHash == null || clientI18nHash === currentSourceHash) return;
  removeKeysWithPrefix(I18N_CACHE_PREFIX);
}
...
export default function IndexPage({
    ...
    async (newLocale: string) => {
      ...
      try {
        const res = await fetch(
          `${baseUrl}/api/translations?lang=${encodeURIComponent(newLocale)}`
        );
  ...
  const calendarUrl = `${baseUrl}/api/calendar?lng=${location.lng}&lat=${location.lat}`;
