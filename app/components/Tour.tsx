...
const SEEN_KEY = "tours:seen";
/** Pre-rename per-step map. Carried over verbatim, then dropped. */
const LEGACY_SEEN_MAP_KEY = "v2:tours:seen";
/** Original boolean "saw the intro" flag, predating per-step tracking. */
const LEGACY_SEEN_FLAG_KEY = "v2:tour:seen";
...
function migrateLegacyIfNeeded(): void {
  ...
  try {
    // v2:tours:seen -> tours:seen. Same shape, so the map carries over as-is and
    // returning users keep every step they had already seen.
    const legacyMap = window.localStorage.getItem(LEGACY_SEEN_MAP_KEY);
    if (legacyMap) {
      if (!window.localStorage.getItem(SEEN_KEY)) {
        window.localStorage.setItem(SEEN_KEY, legacyMap);
      }
      window.localStorage.removeItem(LEGACY_SEEN_MAP_KEY);
    }

    // v2:tour:seen was a bare "1" flag; seed the intro tour as fully seen.
    const legacyFlag = window.localStorage.getItem(LEGACY_SEEN_FLAG_KEY);
    ...
    window.localStorage.removeItem(LEGACY_SEEN_FLAG_KEY);
  } catch {
    // ignore
  }
}
...
const TOUR_READY_FLAG = "__tourReady";
